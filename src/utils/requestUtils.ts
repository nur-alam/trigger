...
export async function fetchUtil(endpoint: string, options: optionsType): Promise<TriggerResponseType> {
...
export async function fetchPostUtil(endpoint: string, options: optionsType): Promise<TriggerResponseType> {
	const { wp_rest_nonce, nonce_key, nonce_value } = config;
	const url = `${endpoint}`;
